...
class PharmacyDetailReview extends StatefulWidget {
  
  String searchId;
  PharmacyDetailReview(this.searchId);

  @override
  _PharmacyDetailReviewState createState() => _PharmacyDetailReviewState();
}

class _PharmacyDetailReviewState extends State<PharmacyDetailReview> {
              ...
              child: Column(
                children: [
                  InkWell(
                    onTap: (){
                    if(bReviewCheck)
                      Navigator.push(context, MaterialPageRoute(builder: (context) => Review(type: 'PHARMACY', id: widget.searchId)));
                    else
                      showDialog(context: context, builder: (context) => OkDialog(text: '이 약국에 대한 진료기록이 없습니다.\n진료기록을 먼저 등록해 주세요.', onOk: (){}));
                    },
                    child: Container(
                      height: 120,
